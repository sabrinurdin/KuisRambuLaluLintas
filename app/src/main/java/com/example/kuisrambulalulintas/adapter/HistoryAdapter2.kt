...
class HistoryAdapter2(
    private var data : List<HistoryEntity2>,
    val onItemCliked : (Int,HistoryEntity2) -> Unit
): RecyclerView.Adapter<HistoryAdapter2.ViewHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HistoryAdapter2.ViewHolder {
        val binding = ItemHistoryBinding.inflate(LayoutInflater.from(parent.context),parent,false)
        return ViewHolder(binding)
    }

    override fun onBindViewHolder(holder: HistoryAdapter2.ViewHolder, position: Int) {
    ...
    inner class ViewHolder(val binding: ItemHistoryBinding) : RecyclerView.ViewHolder(binding.root){
        fun bind(data : HistoryEntity2){
            binding.name.text = data.name
            binding.tvScore.text = "${data.score.toString()}/10"
